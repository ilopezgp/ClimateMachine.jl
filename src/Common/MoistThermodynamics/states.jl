...
Constructs a [`PhaseEquil`](@ref) thermodynamic state from:
 ...
 - `p` pressure
 - `q_tot` total specific humidity
 - `potential_temperature_tol` potential temperature tolerance for saturation adjustment
 - `maxiter` maximum iterations for saturation adjustment
"""
function LiquidIcePotTempSHumEquil_given_pressure(
    ...
    q_tot::FT,
    maxiter::Int = 30,
    potential_temperature_tol::FT = FT(1e-3),
) where {FT <: Real}
...
and, optionally
 - `potential_temperature_tol` potential temperature tolerance for non-linear equation solve
